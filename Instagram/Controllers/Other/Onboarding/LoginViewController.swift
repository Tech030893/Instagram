...
class LoginViewController: UIViewController
{
    ...
    override func viewDidLayoutSubviews()
    {
        ...
        loginButton.frame = CGRect(x: 25, y: passwordField.bottom + 20, width: view.width - 50, height: 52.0)
    ...
    @objc private func didTapLoginButton()
    {
        ...
        AuthManager.shared.loginUser(username: username, email: email, password: password) { success in
            DispatchQueue.main.async
            {
                ...
                {
                    //error occurred
                    let alert = UIAlertController(title: "Login Alert", message: "We were unable to log you in.", preferredStyle: .alert)
    ...
    {
        guard let url = URL(string: "https://help.instagram.com/519522125107875/?helpref=hc_fnav&bc[0]=Instagram%20Help&bc[1]=Policies%20and%20Reporting") else {
            return
        }
        let vc = SFSafariViewController(url: url)
        present(vc, animated: true)
